...
当前源码版本：`1.4.0`；生产环境当前运行测试候选 `v1.4.0t6`，正式 `v1.4.0` 尚未发布。测试候选包含翻译质量与任务恢复、账号注册/邮箱验证码、会员简报与付费墙、EasyPay 兼容自动支付、卡密兑换及 Admin 用户管理。

v1.4.0 的付费设置支持直接以元输入月付/年付划线基准价和现价（最多两位小数）；Admin 自动显示现价占比和优惠百分比，订阅页显示划线原价、绿色优惠标识与现价，订单保存创建时的现价金额。
...
- 旧匿名 double opt-in 的 CSRF、提交和确认入口固定返回 `404`；旧环境开关不能重新开启这些入口，且请求不会写入订阅记录、激活历史 pending 记录或发送确认邮件。一键退订入口继续可用。
- 会员到期后自动停止投递；续费后保留原简报选择并恢复投递资格。Admin 手工管理与会员自助选择使用同一名单。
- 支持一键退订、`List-Unsubscribe` 和 RFC 8058 one-click unsubscribe。
- Admin 可管理会员简报状态；读者只能为自己的已验证注册邮箱启停。会员到期后自动失去投递资格，未决旧刊保留原状态审计并终结，不会在续费后补发。
...
![Admin 用户管理：账号、会员有效期与每日简报操作](docs/screenshots/admin-subscriptions.png)
...
- Admin 可分别以元设置月付/年付划线基准价和现价。例如输入基准价 `36`、现价 `9.9`，Admin 显示“现价为基准价 `27.5%`”，前台显示绿色 `-72.5%` 优惠标识。接口、数据库和订单金额仍使用整数分；订单创建时冻结当时的现价，后续改价不影响已有订单。
- 开通方式支持 EasyPay 兼容网关自动支付和卡密兑换。站点按 `sub2api` 的 API 模式向 `mapi.php` 发起服务端 POST，下单成功后直接跳转网关返回的支付页；公开站点 CSP 只精确放行当前 EasyPay origin，不使用通配域名。订单号固定使用 `news_` 命名空间，异步回调固定为 `/subscribe/api/payment/easypay`。站内订单使用现价 `±0.10`、步长 `0.01` 的唯一尾差，网关回调验签、核对商户号/支付类型/订单号、网关交易号及精确金额后原子开通；重复回调不会重复加时。
...
| 用户管理 | 独立管理登录账号、账号状态、管理员角色、会员计划、剩余天数、到期时间及每日简报状态；支持延期、清除会员、启停简报和发送单账号测试邮件 |
| 付费管理 | 配置 EasyPay API Base、PID、PKey、支付类型、付费墙、月付/年付划线基准价与现价；管理自动支付订单和卡密，查看基准/实付金额、尾差、网关交易号和错误代码 |
