...
| 阶段 | 处理 | 结果 |
|---|---|---|
| 08:00 | 抓取新闻源，去重、评分、选题 | 创建逐篇翻译任务 |
| 翻译 | 逐篇调用 Provider | 成功后保存译文并增量 build、上线 |
| 可恢复上游错误 | 仅当前文章按 `15 秒 → 30 秒 → 60 秒 → 2 分钟 → 5 分钟`退避 | 到期后再次尝试该文章 |
| schema/内容错误 | 保留失败任务和错误代码 | Admin 提供“立即重试”，不让任务消失 |
| 刊期收口 | 全部目标文章完成后执行最终 build | 重新核对会员资格，随后当日刊期只投递一次 |
...
- 正式单篇请求默认硬总时限为 600 秒，任务 lease 为 900 秒；每日/恢复 worker 的系统兜底时限为 90 分钟，覆盖 Luna max 等高推理档位的长响应，同时仍会终止无限挂起。
