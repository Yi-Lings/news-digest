...
def _page(title: str, body: str) -> str:
    return (
        ...
        "box-shadow:0 8px 30px rgba(0,0,0,.25),4px 4px 0 var(--ink);border-radius:6px;"
        "width:100%;max-width:390px;margin:auto;max-height:calc(100vh - 2rem);"
        "max-height:calc(100dvh - 2rem);overflow-y:auto;box-sizing:border-box;"
        ...
        "padding:.45rem .75rem;background:var(--paper);border:1.5px solid var(--rule);"
        "border-radius:4px;margin-bottom:.65rem}"
        ".pay-modal-plan-name{font-weight:700;font-size:.95rem;color:var(--ink)}"
        ".pay-modal-plan-price{font-weight:700;font-size:1.3rem;color:var(--cinnabar);"
        "font-family:Arial,sans-serif}"
        ".pay-modal-tip{font-size:.8rem;color:var(--muted);margin:0 0 .75rem;line-height:1.4}"
        ".pay-modal-options{display:flex;flex-direction:column;gap:.6rem}"
        ...
        "width:2rem;height:2rem;border-radius:50%;font-size:1rem;font-weight:900;"
        ...
        ".pay-channel-text strong{font-size:.92rem;color:var(--ink)}"
        ".pay-channel-text small{font-size:.72rem;color:var(--muted);margin-top:.05rem}"
        ".pay-channel-arrow{font-size:1.05rem;font-weight:700;color:var(--muted);"
        "transition:transform .15s ease}"
        ".pay-channel-btn:hover .pay-channel-arrow{color:var(--ink);transform:translateX(2px)}"
        "@media(max-height:550px){.pay-modal-card{max-height:calc(100vh - 1rem)}"
        ".pay-modal-body{padding:.65rem}.pay-channel-btn{padding:.5rem .75rem}}"
