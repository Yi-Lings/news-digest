...
## 12D. v1.4.0 后续迭代计划：失败句局部重译与质量诊断

状态：`计划已确认，尚未实现、测试、发布或部署`。
...
1. 请求包含目标句原文、前后句只读上下文、段落主题、已有译文和结构化失败 evidence；
...
1. 现有 `translation_tasks` 继续作为文章级任务和租约边界；不新增并发句子 worker，避免同篇文章的
   多句写入竞态。一个文章任务内串行执行句子修复。
2. attempt audit 增加 `kind=sentence_repair` 及目标 `P#S#`，记录 provider、prompt version、
   evidence 摘要和结果分类；不得保存 API key、完整上游响应或未脱敏秘密。
3. 句子修复设置单篇最大句数、最大请求次数和最大总耗时；文章级人工 retry 重置本次预算，
   但不得产生无限自动重试。成功文章不得因刊期级重试被重新生成。
4. cache identity 必须包含 provider、model、prompt version 和 sentence-repair protocol version；
   升级协议时旧局部缓存只能按未命中处理。

### 12D.5 测试与发布门禁

1. 固化 2026-09-04 生产失败样本（Israel、El Nino、Catalina）为脱敏 fixture，覆盖数字缺失、
   日期范围、抓取导航伪段、长文和不可定位错误；不把真实密钥、完整新闻响应或用户数据提交仓库。
2. 单元测试覆盖 evidence 定位、单句 JSON、上下文合并、越界拒绝、并发/租约、缓存隔离和预算耗尽。
   集成测试验证：成功句不变、失败句被替换、全文最终验收失败时不落盘。
3. 回放门禁要求：真实失败 fixture 的误报率为 0；局部修复成功率、请求数和最终全文通过率均有报告。
   全量 pytest、Ruff、`uv build`、Shell/PowerShell/diff 检查全部通过后，才创建新的 `v1.4.0tN`。
4. CI、同一 Release 的 worker/web immutable digest、SQLite backup/integrity check 和 server-push
   完成后才部署；部署本身不触发业务流水线。生产先只重试当前 Terra 失败任务，不补发历史邮件。
...
| 2026-09-01 | 阶段 9.1 | v1.4.0t10 年月日逐级穿透切换日历与日期直达已实现，进入发布部署 | 往期归档彻底摒弃下拉框，实现日视图（周一至周日）/月视图（12月网格）/年视图（年代网格）逐级穿透下钻与向上跳转；左右箭头平滑翻页；卡片下方常驻日期输入框支持直接输入 YYYY-MM-DD 回车直达或智能定位。离线全量 880 passed，Ruff、diff check、uv build 全绿。已获用户授权发布候选 tag v1.4.0t10 并部署至生产环境。 |
| 2026-09-01 | 阶段 9.1 | v1.4.0t11 修复归档可用日期 JSON 注入，全量历史刊期实时联动 | 修复 archive.html 中 data-available-dates 属性双引号截断导致 JS 解析为空数组的问题，改为嵌入独立 application/json script 标签，使全量 38 期历史日报数据在日/月/年三级日历与输入查阅中 100% 完整解析与联动高亮。离线全量 880 passed，Ruff、diff check、uv build 全绿。已获用户授权发布候选 tag v1.4.0t11 并部署至生产环境。 |
| 2026-09-01 | 阶段 9.1 | v1.4.0t12 订阅支付支持弹窗选择微信/支付宝与日历箭头平滑切页动效 | 订阅页点击「立即订阅」后弹出专属支付方式选择对话框（支持支付宝/微信支付双渠道大按钮直达、附带关闭键/ESC/遮罩点击关闭），后端 /order 与结算对账精准适配所选支付渠道；往期归档日历点击左右翻页箭头及上下级穿透时添加丝滑方向位移与缩放动效。离线全量 881 passed，Ruff、diff check、uv build 全绿。已获用户授权发布候选 tag v1.4.0t12 并部署至生产环境。 |
| 2026-09-01 | 阶段 9.1 | v1.4.0t13 响应式浮动支付弹窗自适应缩放与账户订单限高滚动已实现 | 支付弹窗改为响应式浮动卡片（max-width: 390px，自适应视口高度，解除页面锁死，支持双向滚动与小屏自适应），修复 100% 缩放下显示不全问题；我的账户「我的订单」全面接入 240px 限高优雅滚动容器与定制细滚动条，避免完全平铺展开。离线全量 881 passed，Ruff、diff check、uv build 全绿。已获用户授权发布候选 tag v1.4.0t13 并部署至生产环境。 |
| 2026-09-01 | 阶段 9.1 | v1.4.0t14 智谱风格居中固定支付弹窗与微信支付多渠道凭据隔离修复已实现 | ① 修复微信支付报错“订单较多请稍后重试”的根因：payments.config_identity 移除 payment_type 字段哈希冲突，使微信与支付宝共享同一套商户结算凭据且互不干扰；② 弹窗全面升级为智谱 BigModel 平台同款纯居中固定弹窗（套餐原价/优惠活动/实付金额清晰明细卡片、微信与支付宝大按钮、安全保障角标、✕ 与 ESC 顺畅关闭）。离线全量 881 passed，Ruff、diff check、uv build 全绿。已获用户授权发布候选 tag v1.4.0t14 并部署至生产环境。 |
| 2026-09-02 | 阶段 9.1 | v1.4.0t20 归档日期与 GPT 推理强度配置完成，待候选发布 | 往期归档日期输入改为紧凑 `yyyy年mm月dd日` 并保留原生日期选择器、日期直达；Admin“模型接口”新增 GPT 推理强度下拉，配置贯通环境变量、provider 持久化、配置指纹、缓存身份和 OpenAI Chat 请求，仅 GPT 模型发送。完成过时发布文档断言修复后，待执行本地全量门禁、Linux CI、GHCR digest、生产部署与闭环核验。 |
| 2026-09-04 | 阶段 9.2 | v1.4.0t21-t25 生产修复后，句子级重译计划建立 | t21-t23 完成翻译任务重绑定与 provider worker 隔离；t24-t25 修正日期表达和抓取导航数字误报，但 El Nino 仍以 `CONTENT_NUMBER_MISSING` 失败。确认现有实现是整篇 JSON 重试；后续继续使用 `v1.4.0tN` 候选序列，先实现结构化 evidence、句子级局部重译、脱敏生产回放和全量门禁，再发布部署；当前不创建新候选、不改变生产状态。 |
