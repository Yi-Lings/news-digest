...
- schema 12 补齐权益命令与幂等记录、迟到到账/退款/争议台账、有界后台支付对账。
  账户页不再同步查询网关，历史订单支持分页搜索；退款只登记外部凭证及显式扣减天数。
- 注册激活、密码重置与验证码消费同事务；到期账号仍可关闭已有简报偏好。
  人工增时长与付款、兑换共用原子权益合并，清除订阅增加旧值核对。
- 统一 dotenv 解析与重复键拒绝；源配置、Site 投影和数据库生效 revision 可对账恢复。
  Admin 显示配置、业务检查、来源诊断、选择依据与支付金额槽位。
- 新抓取保存发布时间、更新时间、来源与提取诊断；保留短引语、列表、表格内容，
  简讯按时效排序。固定来源样本验证不代替真实模型语义评价，不新增数字或语义硬门。
- 每日 10:30 进行本机一致性备份与隔离恢复校验，保留 14 份；增加 Site/Admin
  healthcheck、持续异常/恢复日志和有界临时数据清理。部署冻结全部写入者。
- 保留 t27 自动单句重译与当前默认 provider 重绑定；不处理往期刊物。
  本次仅发布 prerelease，正式 `v1.4.0` 等待用户观察后决定。
