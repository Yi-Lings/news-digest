...
- 支持空/非字符串正文槽位的自动单句重译，
  全文请求与局部修复共享最多 3 请求、默认 600 秒预算；数字等软信号不触发重译，
  有效全文缓存继续复用。教学字段可降级，不再因标题长度或教学条目数量拒绝正常正文。
- 新增 schema 11 刊期成员与原句快照，完整句级结果、任务成功与构建意图同事务保存；
  当前 provider task 与历史任务隔离，手动 `translate/--redo` 接入同一提交路径，失败保留
  旧译文。正常短段落保留，新抓取路径附 URL 摘要以避免标题重名覆盖。
- 多刊期 manifest、持久发布索引和 generation/result revision 对账；发布后中断可恢复，
  不重新翻译。静态入口不返回内部快照。旧刊期按发布证据及匹配缓存迁移，来源不足保留页面。
- 零收件人显示 `skipped/no_eligible_recipients`，不恢复退订、不补发；旧错误完成状态保守校正。
  部署先停 Site/Admin，备份后离线迁移；增加副本核对脚本。PR/main 复用 tag 离线校验。
- 单篇预算上限 3600 秒，lease 额外保留 60 秒；systemd 总上限调整为 24 小时，覆盖多篇
  持久重试，不改变默认 600 秒/3 请求与每刊期三次自动执行上限。

- 以下为 t20 时的历史发布目标。该 tag 创建 GitHub prerelease，明确不成为
